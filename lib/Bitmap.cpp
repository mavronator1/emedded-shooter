                    ...
                    int s0) const
{
    ...
    int sf = s0;    // scaling factor 
    
    // each pixel inside sprite array is drawn sf number of times
    ...
    for (unsigned int bitmap_row = 0; bitmap_row < _height; ++bitmap_row)
    {
        ...
        // moves the row index along to allocate sf extra pixels
        for(int r = 0; r < sf; r++) {
            unsigned int screen_row = y0 + bitmap_row + (bitmap_row*(sf-1)) + r;        
        ...
        for (unsigned int bitmap_col = 0; bitmap_col < _width; ++bitmap_col)
        {
            ...
            // moves the column index along to allocate sf extra pixels
            for(int c = 0; c < sf; c++) 
            {
            
            int screen_col = x0 + bitmap_col + (bitmap_col*(sf-1)) + c;
